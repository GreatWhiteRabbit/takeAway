    ...
    <div class="login">
      ...
      <div class="login-box">
        <div class="login-input">
          <span >用户:</span>
          <input type="text" placeholder="邮箱/用户名/手机号" v-model="sellerid">
        </div>
        <div class="login-input">
          <span >密码:</span>
...
export default {
  ...
  methods: {
    ...
    log(){
      axios({
        method:'get',
        url:this.HOST+"users/sellerLogin",
        params:{
        ...
        if(res.data===true){
          this.$store.state.sellerid=this.sellerid;
          this.$router.push({path:'/sellerProfile',query:{sellerid:this.sellerid}});
        } else {
    ...
    reg(){
      ...
      axios({
        method:'get',
        url:this.HOST+"/users/sellerRegister",
        params:{
          userid:this.sellerid,
          password:this.password
        }
